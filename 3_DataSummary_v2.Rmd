...
instruments <- stops %>%
  ...
  mutate(
    Instrument = c("MA200", "Li-Cor", "M903", "DiSCmini", "P-TRAK, Screen", "NanoScan", "CAPS", "P-TRAK")
  )
...
pollutant_levels <-  c("UFP (pt/cm3), 10-420 nm", "UFP (pt/cm3), 10-700 nm", "UFP (pt/cm3), 20-1,000 nm", "UFP (pt/cm3), 36-1,000 nm", "BC (ng/m3)", "NO2 (ppb)", "PM2.5 (ug/m3)",  "CO2 (ppm)")

pollutant_levels2 <-  c("UFP (pt/cm3)",  str_subset(string =  pollutant_levels, pattern = "UFP", negate = T))
...
maps instead of boxplots like above

```{r} 
#location_quant %>%
location_quant2 <- stops %>%
  #filter(!grepl("UFP", pollutant)) %>%
  #calc stats for alternative boxplots
  group_by(pollutant, ufp_range_nm, location, #location_no
           ) %>%
...
#i=1
   ...
   p[[i]] <- ggmap(ggmap = map0, darken = c(.5, "white")) +
    #monitoring area
    geom_sf(data=monitoring_area_shp, inherit.aes = F, 
            aes(fill = "Monitoring Area"), show.legend = F,
            alpha=0.1, 
            #reduce/eliminate outline
            lwd = 0.1) +
    
    #annual averages
    geom_point(data=filter(location_quant2, pollutant == non_ufp_p[i]),
             aes(x=longitude, y=latitude, col=value), 
             inherit.aes = F, size=2)  +
    scale_color_gradient(name = "IQR Conc",
                        low = "yellow", high = "red") +
    facet_grid(Quantile~pollutant)  + 
...
ggarrange(plotlist = p#, #ncol=4, #align = "h"  
          ) %>%
  annotate_figure(left = "Latitude", bottom = "Longitude")

ggsave(file = file.path(image_path, "SI", "visit_maps_iqr_non_ufp.png"),
         height = 11, width = 7)
...
ggmap(ggmap = map0, darken = c(.5, "white")) +
  ...
  #annual averages
  geom_point(data=filter(location_quant2, pollutant == ufp_p),
           aes(x=longitude, y=latitude, col=value), 
           inherit.aes = F, size=2)  +
  scale_color_gradient(name = "IQR Conc",
                      low = "yellow", high = "red") +
...
ggsave(file = file.path(image_path, "SI", "visit_maps_iqr_ufp.png"),
         height = 14, width = 8)
...
stops %>%
    mutate(month =month(time, label = T),
           ufp_range_nm = ifelse(ufp_range_nm == "-", "NA", ufp_range_nm)
           ) %>%  
    #calc stats for alternative boxplots
    ...
    ggplot(aes(x=season, #x=month, 
               fill=ufp_range_nm
               #fill= interaction(ufp_range_nm, pollutant)
               )) +
...
stops %>%
  ...
  mutate(hour = factor(hour, levels = c(4:23#, 0
    ...
    #calc stats for alternative boxplots
    group_by(pollutant2, ufp_range_nm, hour) %>%
    alt_boxplot(var = "value") %>%  
    
    ggplot(aes(x=hour, fill=ufp_range_nm)) +
    geom_boxplot(aes(ymin=Qmin, lower=Q25, middle=Q50, upper=Q75, ymax=Qmax), stat = "identity") + 
     
  facet_wrap(~pollutant2, scales="free", nrow = 4) +
  theme(legend.position = "none") +
...
```{r}
var_df %>%
  gather(pollutant, percent, 'BC (ng/m3)':'UFP (pt/cm3), 36-1,000 nm') %>%
...
```{r}
var_df %>%
  #select(-c(contains(c("10-420", "10-700", "36-1,000")))) %>% 
  gather(pollutant, percent, 'BC (ng/m3)':'UFP (pt/cm3), 20-1,000 nm') %>%
  filter(Variable != "total") %>%
  mutate(
...
ggsave(file = file.path(image_path, "anova.png"), height = 9, width = 9)                 
...
annual %>%
    mutate(ufp_range_nm = ifelse(ufp_range_nm == "-", "NA", ufp_range_nm),
           pollutant = factor(pollutant, pollutant_levels2)
           ) %>%
...
trh0 %>%
  ...
  kable(caption = "Distribution of site-specific annual average temperature and relative humidity values.", # N = 309 sites.",  # or N=9,047 stops
        digits = 0
        ) %>%
  kable_styling()
...
ptrak <- annual %>%
  ...
  gather(variable, value, contains("pnc_")) %>% #View() 
  variable_relabel()
...
ggsave(file.path(image_path, "SI", "map_ptrak_tot_and_bins.png"),  
       height = 7, width = 12
       )
...
total_pollution <- annual %>%
  ...
  filter(!(grepl("UFP", pollutant) & !grepl("20-1,000", ufp_range_nm)
             #ufp_range_nm != "20-1,000"
           )) %>% # View()
...
```{r}
geo_t <- read_csv(file.path("~", "OneDrive - UW", "Documents", "School", "PhD", "Dissertation", "TRAP R Project", "Output", "Aim 2", "Tables", "0. Geocovariates", "cov_descrip.csv")) %>%
