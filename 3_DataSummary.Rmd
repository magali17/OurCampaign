...
#   "Data", "Output", "GIS", "monitoring_area_shp2.rda"
#   #"..", "GIS", "Shapefiles", "Monitoring", "monitoring_area_filled_in.shp"
#   )
#   ) %>%
#   #convert from 4269 
#   st_transform(crs_deg)
...
ptrak_bin_range <- c("20-50", "50-1,000")
...
stops <- stops0.1 %>%
  ...
  filter(!variable %in% c("pnc_20_50", "pnc_screen")) #%>%
  # mutate(
  #   instrument = ifelse(ifelse(grepl("pnc_", variable), "P-TRAK", "NanoScan"),)
  # )
...
### --> reduce # of bins when have 13th bin? 
...
#i=1
  p <- lms_bins %>%
    ...
    geom_point(aes(size=Buffer),
             shape=1,
             alpha=my.alpha) +
    facet_wrap(~ufp_range_nm, scales = "free") +
    geom_vline(xintercept=0, linetype=2, alpha=my.alpha) +
    labs(y = "Geocovariate",
         x = "Coefficient Estimate",
         col = "UFP\nSize\nRange (nm)"
  ...
  ggsave(file.path(image_path, "SI", paste0("lasso_bin_",bin_instruments[i], ".png")), height = 13, width = 13)
...
```{r}
# # calculate UFP percentiles after binning, based on lasso results
# annual_bins %>% 
#   distinct(variable, ufp_range_nm)

ufp_bin_pctl <-annual_bins %>%
  filter(!variable %in% c("median", "pnc_20_50", "pnc_screen")) %>%
...
print("UFP percentiles by particle size range. Based on binned NanoScan concentrations.")
