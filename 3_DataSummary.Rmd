...
test <- stops %>%
  ...
  select(time, stop_id, variable, value) %>%
  spread(variable, value) %>%
  drop_na() %>%
  mutate(
    pnc_20_50_nm = pnc_noscreen - pnc_screen
  ) %>%
  gather("variable", "value", pnc_20_50_nm)
    ...
    p <- stops %>%
      ...
      geom_boxplot(aes(xmin=Qmin, xlower=Q25, xmiddle=Q50, xupper=Q75, xmax=Qmax), stat = "identity") +
       facet_wrap(~pollutant+ufp_range_nm, #scales="free", 
        ...
        labs(y= "Site No.",
             x= "Value",
             col = "Particle Range (nm)"
             )  +
      scale_y_discrete(breaks = seq(10,310, 10)) #+ scale_x_log10()
    
    if(!grepl("UFP", unique_pollutants[i])) {
      p <- p + theme(legend.position = "none") + scale_x_log10()
        # # for ref if on diff x axes  
        # geom_vline(xintercept = 25e3, linetype=3, alpha=0.5)
      }  
