...
### --> separate bin counts from total counts 
...
route_dist <- routes_shp %>%
  ...
  # mutate(
  #   length_mi = specify_decimal(length_mi, 1),
  #   length_km = specify_decimal(length_km, 1),
  #   
  #   length_mi_km = paste0(length_mi, " (", length_km, ")")
  # )  
  
  
#route_dist %>% kable() %>% kable_styling()
...
on_road <- data.frame(
  Route = c(seq(1:9), ""),
  no_road_segments = NA,
  median_iqr_drive_time_per_drive_hrs = NA,
  total_drive_time_hrs = NA
)
...
route_stats1 %>%
  ...
  kable(caption = "Route statistics. Number of sampling dates includes makeup routes, which may have been slightly altered.", 
        col.names = c("Route", "No. Stops", "No. Sampling Dates", "Distance (mi)","Distance (km)", "Total Distance (mi)", "Total Distance (km)", names(on_road)[-1]),
        digits = 0, 
        format.args = list(big.mark = ",")
        ) %>%
  kable_styling() #%>%
    # add_footnote(c(
    #   "Total route driving distance is estimated from the route distance and the number of sampling dates. The exact distance varied based on makeup routes, route deviations, etc.",
    #   "Road segments were approximately ____ m long."
    #                 ))
...
stops %>%
  ...
  #'Max/Min Ratio' = format(Max/Min, digits = 2, big.mark = ","),
  'IQR/Median' = format((Q75-Q25)/Median, digits = 1),
  ) %>%
  
  mutate(Pollutant = gsub(", ", "\n", Pollutant)) %>%
  mutate_at(
    vars(N:Max), ~ifelse(grepl("UFP|CO2|BC", Pollutant), 
                           format(., digits=0, scientific = F, big.mark = ","),
                           format(., digits=2, scientific = F, big.mark = ",")
                           )
  ) %>%
  kable(caption = "Distribution of median visit concentrations pollutant concentrations at monitoring sites (N = 309 sites x ~ 29 visits/site)") %>%
  kable_styling()
...
location_quant %>%
  mutate(
    pollutant = as.character(pollutant),
    pollutant = ifelse(grepl("UFP", pollutant), paste0(pollutant, "\n", ufp_range_nm), pollutant)
         ) %>%
  group_by(Pollutant=pollutant) %>%
  summarize(
    Min = min(iqr),
  ...
  kable_styling() %>%
  add_header_above(c("Pollutant", "Within Site Variability of Median Visit Conc's" = 2, "Between Site Variability of Annual Avg Conc's" = 1))
...
location_quant %>%
  ...
  geom_vline(data=filter(visit_conc, !grepl("UFP", pollutant)), aes(xintercept = Median), alpha=0.4, linetype=1) +
  geom_vline(data=filter(visit_conc, !grepl("UFP", pollutant)), aes(xintercept = Q05), alpha=0.4, linetype=2) +
    geom_vline(data=filter(visit_conc, !grepl("UFP", pollutant)), aes(xintercept = Q95), alpha=0.4, linetype=2) +
...
# stops %>%
#    mutate(
#     pollutant2 = as.character(pollutant),
#     pollutant2 = ifelse(grepl("UFP", pollutant2), paste0(pollutant2, "\n", ufp_range_nm), pollutant2)
#          ) %>%
#   group_by(pollutant2, pollutant, season) %>%
#   summarize(
#     median = median(value)
#   ) %>%
#   #spread(season, median)
#   ggplot(aes(x=season, y=median, col=pollutant2, group=pollutant2,
#              )) + 
#   facet_wrap(~pollutant, scales = "free") +
#   geom_point() +
#   geom_line() + 
#   labs(y = "Median Visit Conc")
...
stops %>%
  ...
  facet_wrap(~pollutant+ufp_range_nm, scales="free", nrow = 3) +
    labs(y= "Value",
         x= "Date",
         col = "UFP Range (nm)"
         )  
...
var_df %>%
  ...
  t() %>%
  kable(caption = "Percent of variability in visit concentrations explained by spatial and temporal factors (out of 100%). Values are based on separate Analysis of Variance (ANOVA) models for each pollutant. Total and error degrees of freedom (DF) vary by pollutant.", 
        digits = 1) %>%
  kable_styling()
...
### --> add "route" to ANOVAS. shouldnt see any effect. only need to include results if something comes up 

### --> could classify a route as beginning/middle/end - is there any structure in thiss?

* this is a proxy for location and time
...
#ERROR: 2nd axis does not work w/ ggarrange well
...
for (i in seq_along(unique_pollutants)) {
  ...
  p[[i]] <- annual %>%
    filter(pollutant == unique_pollutants[i]) %>%
    ggplot(data=., aes(x=pollutant, y=value, col=ufp_range_nm)) + 
    facet_wrap(~pollutant, scales= "free") + 
    geom_boxplot() +
    scale_y_continuous(sec.axis = sec_axis(trans = ~./min_val,
                                           name = 'Relative to Min Value'),
  ...
  if(!grepl("UFP", unique_pollutants[i])) {
...
ggarrange(plotlist =  p, ncol = 2, nrow = 3 #, 
          #common.legend = T, #common.legend = c(3)
           )
...
```{r}
## Variograms (spatial variability)

### --> move NO2

### --> gstat will pick best one if you give it multiple empirical models, but parameters will have diff meanings

### --> "note that this is before we’ve removed important predictors of pollution"

```
 
```{r, eval=F}
# source: https://github.com/liannesheppard/ENVH556_2021course/blob/main/Week8LabGeostatistics.Rmd

annual_sf <- st_as_sf(annual, coords=c('longitude','latitude'), crs=crs_deg) %>%
  st_transform(crs = crs_m) 

# vgm_cloud <- variogram(value~1, 
#                        data = filter(annual_sf, variable=="ns_total_conc"), 
#                        cloud=TRUE)
...
for(i in seq_along(uniqe_variables)) {
  #i=1
  df = filter(annual_sf, variable==uniqe_variables[i]) %>%
    mutate(
      pollutant = ifelse(grepl("UFP", pollutant), paste0(pollutant, ", ", ufp_range_nm), as.character(pollutant))
    )
    
  # cutoff is automatically set to 1/3 bbox diagonal distance
  vgm_bin <- variogram(value~1, data=df)
  
  # fit a variogram model; using exponential fit
  v.fit <- fit.variogram(vgm_bin, vgm(c("Exp")))
  ##for plot
  v.fit2 <- v.fit %>%
    as.data.frame() %>% 
  ...
  preds = variogramLine(v.fit, maxdist = max(vgm_bin$dist))
   
  p[[i]] <- ggplot(data=vgm_bin, aes(x = dist, y = gamma)) +
    geom_point(shape = "o", alpha = 0.6) +
    geom_line(data = preds, aes(col="Exponential")) +
    geom_smooth(se = FALSE, aes(col = "Loess")) +
    labs(x = "Distance (meters)", 
         y = "Semi-variance",
         col = "Fit",
         title = paste0(unique(df$pollutant))
         )  + 
    annotate(geom = "table", label = list(v.fit2), 
             x = Inf, y = -Inf) #+ geom_text(aes(label=np))
  
 #p[[i]]
 
}

print("Binned semi-variogram for annual averages with overlaid curve fits")
#print("numbers are the number of point pairs for this estimate")

ggarrange(plotlist = p, common.legend = T, legend = "bottom")
...
map0_terrain <- suppressMessages(get_stamenmap(
  bbox = bbox, maptype = "terrain"))
...
ufp2 <- str_subset(unique(annual$pollutant2), "UFP" ) %>% sort()

for(i in seq_along(ufp2)) {
   #i=1
...
ggsave(file.path(image_path, "map_ufp.png"),  height = 13, width = 11)

```


same as above but for SI - same legend
...
ggsave(file.path(image_path, "SI", "map_ufp_same_legend.png"),  height = 13, width = 11)
...
* using NanoScan UFP measures (10-420 nm)  s
...
### --> change shape; make lightest color lowest conc, not avg conc

```{r}
  
total_pollution <- annual %>%
  #only keep 1 UFP measure
  filter(!(grepl("UFP", pollutant) & ufp_range_nm != "10-420")) %>%
...
geo_t %>%
  kable(caption = "Available geographic covariates (geocovariates) at monitoring sites.",
    row.names = T) %>%
  kable_styling()%>%
  add_footnote("see MESA DOOP for additional details") 
...
lms %>%
  ...
  geom_point(data = lms[is.na(lms$buffer),],
           alpha=my.alpha,
           aes(shape="")) +
  geom_vline(xintercept=0, linetype=2, alpha=my.alpha) +
    facet_wrap(~pollutant, labeller = "label_both",
               #nrow = 1, 
               scales = "free") +
  labs(y = "Geocovariate",
       x = "Coefficient Estimate",
       shape= "non-buffer", 
       col = "UFP Size Range (nm)"
       ) +
  theme(legend.position = "bottom")

ggsave(file.path(image_path, "SI", "lasso.png"), height = 11, width = 12)
...
## Other BC & UFP studies

### --> Change our results if use this

```{r}



ufp0 <- readxl::read_xlsx(file.path("~/Documents/School/PhD/Dissertation/References", "Mean UFP & BC comparison.xlsx")) %>%
  ...
  mutate(Mean = ifelse(is.na(Mean), yes = Median, no = Mean),
         ...
         Sampling_type = ifelse(grepl("continuous", Sampling_type, ignore.case = T), "Continuous Fixed Site", ifelse(grepl("mobile", Sampling_type, ignore.case = T), "Mobile - No Stops", "Mobile - Stops" ))
...
## bc
bc0 <- readxl::read_xlsx(file.path("~/Documents/School/PhD/Dissertation/References", "Mean UFP & BC comparison.xlsx"), sheet=2) %>%
  #only include studies that have been fully entered into excel or qualify
  filter(Include_in_analysis == TRUE) %>%
  select(-Include_in_analysis) %>%
  mutate(
