...
### --> do this. at stop level? otherwise PMPTSCREEN_94 has some low values that don't correlate wel w/ PMPTSCREEN_3
...
```{r, fig.height=12, eval=F}
#same as above, but as separate facets w/ diff legends
...
###--> see if scan3 has temporality issues? does poor performance occur later on in the study?
...
* dropped SCAN 3 since    
  * it was a much older instrument
  * we only collected a few extra days of data with it 
...
dt_stops4 <- dt_stops3 %>%
  ...
  #drop poor performing instruments
  filter(!instrument_id %in% c("CO2_19", "PMSCAN_3"))

# note, this dataset could still have
dt3 <- dt2 %>%
  filter(!instrument_id %in% c("CO2_19", "PMSCAN_3"))
...
### --> check that tz change is correct 
...
dt_stops3_colo <- dt_stops3 %>%
  ...
  ### --> change from UTC to los angeles
  
  #drop seconds from time stamp (for DOE merging). change default tz of UTC
...
### --> update calibration curve if Dave has updates   
### --> calibrate at 2-min level? 

### --> see DOE neph calibration curves 
...
###--> usae same legends for facets 

### --> neph PM2.5 estimates are Very low. multiplying slope by 10 makes thigns look better. the neph (bscatx10^-4) and PM2.5 (ug/m3) units should be correct. Does this suggest fitting a different calibration curve? Do DOE's calibration curves differ that much from ours?
...
### --> are neph ranges: -.18-0.22 not that different ?
...
### --> see when did we visit neph DOE sites?
