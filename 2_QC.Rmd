...
* inspected time series data during times when extreme instrument concentrations (min & max conc) were observed. Readings were compared against other instrument readings (i.e., were other instruments also reporting elevated levels?)   
  * dropped one instance of CO2_19 (backup instrument) readings when readings were dissimilar from other instruments. Note, this instrument is fullu dropped later.
...
all_stops <- length(unique(final_data$stop_id)) #value is same for original_data
...
* NO2   ____
* BC was probably dropped for having negative concentrations (outside the instrument measuremement range)   
* CO2    
* Neph (same as BC?)     
* discmini (10-700)    

### --> redo this to reflect Amanda's suggestions 
...
full_join(s0, s1) %>%
  ...
  mutate(
    ...
    percent_stops_dropped = stops_dropped/original_stops*100
  ) %>%
  kable(caption = "Mobile monitoring stops", 
        digits = 2, 
        col.names = c("Pollutant", "Original Stops", "Final Stops", "Stops Dropped", "Percent Stops Dropped"), 
        format.args = list(big.mark=', ')
        ) %>%
  kable_styling()
