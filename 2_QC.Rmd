...
calibration_data_2min <- calibration_data %>%
  select(site, instrument_id, analyte, level, conc, unit, duration, runname, time, variable, value) %>%
...
calib_instrument_ids <- calibrations %>%
...
### --> how do these look? drop more data? 
```{r}
#particles
for(i in seq_along(pt_instrument_ids)) {
  #i=1
...
### --> nephs read slightly higher as attenuation increases? 
...
### --> START HERE. WIDE ERROR   
### --> use calibration_data_2min instead of room_colo  

```{r}
#header names in both sources of data: stop and room data 
common_names <- intersect(names(dt_stops3), names(calibration_data))

dt_stops3_colo <- dt_stops3 %>%
  #these are never collocated
  filter(!instrument_id %in% c("PMPTSCREEN_2", "PMPTSCREEN_93")) %>%
  select(common_names) %>%
  rbind(calibration_data_2min[common_names])   
...
# dt_stops3_colo %>% slice(81255, 81424)
# calibration_data_2min %>% filter(time == "2020-02-11 17:20:00", instrument_id=="CO_1")
