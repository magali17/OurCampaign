...
* added 10-second lag to all instruments to account for the time required for a volume of air to travel from the sampling inlet to each instrument 

* dropped readings with error codes suggesting inaccurate field measurements   
  * MA200 (BC): pump flow and optical saturation errors     
  * NO2: readings during automatic baseline checks     
...
* inspected time series data during times when extreme instrument concentrations (min & max conc) were observed. Readings were compared against other instrument readings (i.e., were other instruments also reporting elevated levels?)   
  * dropped one instance of CO2_19 (backup instrument) readings when readings were dissimilar from other instruments. Note, this instrument is fully dropped later.
...
* compared & calibrated collocated duplicate instruments 
  ...
  * we calibrated some duplicate instruments a second time:    
    ...
    * if there was no primary instrument, however, readings were calibrated to the average of both instruments based on their collocations    
      * discminis   
    * calibrate backup instruments when they deviate from the primary instrument **and backup instruments were used on their own at the begining of the study** using collocation data from early on in the study     
      * calibrate NanoScan (ns) 3 to ns 5 using data collected before __2019-04-01__ since 3 tended to read low and ns 3 values after this were not used to estimate site annual averages anyways    
        * calibrations were done separately for total counts and for bin-specific counts   
          * also checked that instruments were generally in agreement in their bin readings before 2/1/2020 (after which we know NS 3 starts to act strange)      
      * calibrate NO2_1 to NO2_2 using data collected before __2019-04-01__.     
        * Note that some NO2_1 values become negative, but 2-minute AQS collocations were more accurate    
    
* calibrated neph readings to estimate PM2.5 using Copper's calibration curve

* checked that there were no instrument trends occurring over the course of a samplign day (e.g., increasing instrument readings as a funciton of time)   

* AQS data comparisons   
  * checked that Copper's calibration curve was similar to the one fit using DOE data for the study period. Estimates would thus be similar regardless of what curve was used.
...
* Temp & relative humidity      
  * all 2-min median readings were within the instrument range    
  * compared collocated instruments - they looked good    
  * only kep readings from the primary instrument       
  * later use these to calculate annual average values for each site and the entire study area      
  * could think about comparing these to those from collocated AQS sites in the future    
...
# note: BC_0263 is different from BC_0063. BC_0263 was used at AQS sites overnight.
...
```{r}
drop_ns <- c("ns_geo_mean_nm", "ns_gsd", "ns_mean_nm"#, "ns_mode_nm"
             )

ns0 <- readRDS(file.path("Data", "Original", "ns_bin_data.rda")) %>%
  rename(time = timeint) %>%
  mutate(date = ymd(substr(time, 1, 10))) %>%
  filter(
    #drop instruments w/ few readings (only want a primary and backup instrument)
  ...
  mutate(primary_instrument = ifelse(instrument_id %in% primary_instruments, "Primary", "Backup"),
         ...
         # ? rename bins 
         variable = gsub("ns_|_nm", "", variable),
         variable = gsub("_", ".", variable)
         )

#arrange bins & other variables at the end
ns_levels <- str_subset(unique(ns0$variable), "[0-9]+") %>%
  as.numeric() %>%
  sort() %>%
  #do this instead of as.character() to keep the ".0" in "154.0"
  format(., nsmall = 1) %>% trimws() %>% #as.character() %>%
  c(., str_subset(unique(ns0$variable), "[0-9]+", negate = T))

 ns0 <- ns0 %>%
   mutate(variable = factor(variable, levels = ns_levels))
...
```{r}
trh0 <- readRDS(file.path("Data", "Original", "weather_data.rda")) %>%
  rename(time = timeint) %>%
  mutate(date = ymd(substr(time, 1, 10))) %>%
  filter(
    # #drop instruments w/ few readings (only want a primary and backup instrument)
...
trh0 <- trh0 %>%
  mutate(primary_instrument = ifelse(instrument_id %in% primary_instruments_trh, "Primary", "Backup"),
         primary_instrument = factor(primary_instrument, levels = c("Primary", "Backup")),
         )
...
dt0 %>%
  ...
  summarize(
      sampling_days = n(),
      start_date = min(runname),
      end_date = max(runname)
    ) %>%
  ungroup() %>%
  add_row(variable="OVERALL MEAN", sampling_days=mean(.$sampling_days)) %>%
  kable(caption = "Initial drive days (before data cleaning)", 
        digits = 0
        ) %>%
  kable_styling()
...
* TRH_1 was the primary instrument  

```{r}
#plot   
trh0 %>%
  distinct(date, variable, instrument_id) %>%  
  ggplot(aes(x=date, y= interaction(variable, instrument_id), col=variable)) + 
  geom_point() + 
  labs(y = "variable.instrument_id")
...
* there are no "status" (error) variable values for these measures  
...
```{r}
#are any stops dropped as a whole as a result of dropping individual readings w/ flags? - no
t1 <- dt0 %>%
  #filter(variable %in% c("no2", "ma200_ir_bc1")) %>%
  group_by(variable) %>%
  summarize(
    stops_dt0 = length(unique(stop_id))
  )
...
t2 <- dt %>%
  ...
  group_by(variable) %>%
  summarize(
    stops_dt = length(unique(stop_id))
  )
 
left_join(t1, t2) %>%
  mutate(stops_dropped = stops_dt0 - stops_dt) %>%
  kable(caption = "number of stops dropped as a result of BC and NO2 flags") %>%
  kable_styling()
...
* see a lag that occurs in instrument response to a true reference concentration when looking at time series plots. need to add a lag to start and end of each reference reading.
...
calibration_curves %>%
  gather("term", "estimate", intercept, slope) %>%
  ...
  kable(caption = "Distribution of calibration curve coefficient estimates for conc_instrument ~ conc_reference*B1 + B0. N = number of calibration days. 'conc' term is the slope. A no intercept model was fit to instrument NO2_2, which reset after each zero reading.", 
        digits = 2, 
        col.names = c("Pollutant", "Instrument ID", "Term", "N", "Min", "Median", "Max")
...
low_value <- readRDS(file.path("Data", "Output",  "stops_distribution_table.rda")) %>%
  select(variable,
    #instrument_id, 
         low = Q05)
...
```{r}
#TEST  -same as below but points 

print("2-min median instrument readings during zero checks")
...
calibrations %>%
  ...
  left_join(low_value) %>% #View()
  #add if primary instrument
  mutate(Instrument = ifelse(instrument_id %in% primary_instruments, "Primary", "Backup"),
         Instrument = factor(Instrument, levels = c("Primary", "Backup"))
         ) %>%
  variable_relabel() %>%
  
  ggplot(aes(y=instrument_id, 
             )) + 
  facet_wrap(~variable +ufp_range_nm, scales="free") +
  geom_vline(xintercept = 0, linetype=2, alpha=0.5) +
  #geom_point(aes(x=value), alpha=0.3) +
  geom_jitter(aes(x=value, col=Instrument), alpha=0.3) +
  
  #geom_boxplot(stat = "identity", aes(xmin=Qmin, xlower=Q25, xmiddle=Q50, xupper=Q75, xmax=Qmax)) +
  
  # a low on-route value (for comparison)
  geom_vline(aes(xintercept = low, linetype=paste0("Low Stop Conc\nQ", low_q)),
             col="red"
             ) +
  labs(x = "Value",
       y = "Instrument ID",
...
ggsave(filename = file.path(image_path, "SI", "zero.png"), width = 10, height = 6)
...
calibrations %>%
  ...
  ggplot(aes(y=instrument_id, linetype=Instrument)) + 
  facet_wrap(~variable +ufp_range_nm, scales="free") +
  geom_vline(xintercept = 0, linetype=2, alpha=0.5) +
  geom_boxplot(stat = "identity", aes(xmin=Qmin, xlower=Q25, xmiddle=Q50, xupper=Q75, xmax=Qmax)) +
  
  # a low on-route value (for comparison)
  geom_vline(aes(xintercept = low, col=paste0("Low Stop Conc\nQ", low_q)), #size=3
             ) +
  labs(x = "Value",
       y = "Instrument ID",
       col=""
...
# ggsave(filename = file.path(image_path, "SI", "zero.png"), width = 10)
...
calibrations %>%
  ...
  ggplot(aes(y=instrument_id, fill=date, linetype=Instrument)) + 
...
calibrations %>%
  ...
  mutate(filter_to = ifelse(grepl("manifold", notes), "manifold", "instrument"),
         date = substr(runname, 1,10),
         Instrument = ifelse(instrument_id %in% primary_instruments, "Primary", "Backup"),
         Instrument = factor(Instrument, levels = c("Primary", "Backup")),
         #calculate 2-min medians 
  ...
  group_by(variable, instrument_id, #date, 
           filter_to, Instrument) %>%
  alt_boxplot(var = "value") %>%

  ggplot(aes(y= instrument_id, fill=filter_to, linetype=Instrument)) + 
  facet_wrap(~variable, scales="free") +
  geom_vline(xintercept = 0, linetype=2, alpha=0.5) +
  geom_boxplot(stat = "identity", aes(xmin=Qmin, xlower=Q25, xmiddle=Q50, xupper=Q75, xmax=Qmax)) +

  scale_fill_brewer(palette = "Purples") +
  labs(col="", y = "Instrument ID")
...
second-level: comparison of original and final second-level data used to calculate stop medians
...
full_join(s0, s1) %>%
  variable_relabel() %>%
  mutate(variable = ifelse(grepl("UFP", variable), paste0(variable, ", ", ufp_range_nm, "nm"), as.character(variable) )) %>%
  select(-ufp_range_nm) %>%
  rbind(c("TOTAL", sum(.$original_samples), sum(.$final_samples))) %>%
  mutate_at(vars(-variable), ~as.numeric(.)) %>%
  mutate(
    samples_dropped = original_samples - final_samples,
    percent_samples_dropped = samples_dropped/original_samples*100
  ) %>%
  
  kable(caption = "Mobile monitoring samples", 
        digits = 2, 
        col.names = c("Pollutant", "Original Samples", "Final Samples", "Samples Dropped", "Percent Samples Dropped"), 
...
# Calculate Site medians/means (for each instrument)
...
```{r, eval=F}
# check that mean and median values are different
dt_stops %>%
...
dt_stops %>%
  ggplot(aes(y=mean_value, x=value)) + 
  facet_wrap_equal(~variable, scales="free") + 
  geom_point(alpha=0.2) +
  geom_abline(slope = 1, intercept = 0) + 
  labs(x = "median_value ('value')")

```

-same as above but for the nanoscan bins
...
-same as above for temp, rh data 
...
- same as above for temp, rh

* all temp, rh values are within the instrument range 

```{r}
instrument_range_trh <- data.frame(
  variable = unique(trh_stops$variable),
  Min = c(0, -4),
  Max = c(95, 158)
)
...
trh_stops %>%
  ...
  left_join(instrument_range_trh) %>%  
  mutate(below_min = value < Min,
         above_max = value > Max
         ) %>% #View()
  group_by(variable) %>%
...
#only keep appropriate nanoscan bin data
ns_stops2 <- dt_stops2 %>%
...
t1 <- dt_stops %>%
  group_by(variable) %>%
  summarize(dt_stops = length(unique(stop_id)))

t2 <- dt_stops2 %>%
  group_by(variable) %>%
  summarize(dt_stops2 = length(unique(stop_id)))

left_join(t1, t2) %>%
  mutate(stops_diff = dt_stops - dt_stops2 ) %>%
  kable(caption = "Stops before and after updating data. stops were dropped b/c: 1. readings were outside instruemtn range, 2. NanoScans and non-screened P-TRAKS readings < 300 pt/cm3, 3. screened P-TRAKS readings < 100 pt/cm3. This table is almost identical to the final table, other than for CO2 - readings are dropped late rb/c they are different from other instrument readings?") %>%
  kable_styling()
...
t2 <- dt_stops3 %>%
  group_by(variable) %>%
  summarize(dt_stops3 = length(unique(stop_id)))

left_join(t1, t2) %>%
  mutate(stops_diff = dt_stops2 - dt_stops3) %>%
  kable(caption = "Stops before and after updating data. backup CO2 readings are dropped b/c they are different from collocated backup instrument. # of stops is the same b/c the primary instrumetn will be used.") %>%
  kable_styling()
...
```{r}
#note: not including lab calibration data since adding calibration_data_2min causes issues b/c: a) there can be multiple diff reference concentrations during any given time; and b) it's likely but not guaranteed that duplicate instruments were measuring the same reference concentrations at the same time
...
-same as above for NS data 

```{r}
ns_stops2_colo <- ns_stops2 %>%
  select(common_names, variable)  

ns_stops2_colo_wide <- ns_stops2_colo %>%  
  spread(instrument_id, value) %>%
  mutate(site = substr(runname, 12,14)) %>%
  drop_na()
...
```{r}
trh_stops_colo <- trh_stops %>%
  select(common_names, variable)  

trh_stops_colo_wide <- trh_stops_colo %>%  
  spread(instrument_id, value) %>%
  mutate(site = substr(runname, 12,14)) %>% #View()
  drop_na()
...
similar to above, but for manusctipt

```{r, fig.height=14}
p <- list()

p[[1]] <- dt_stops3_colo_wide %>%
  drop_na(BC_0066, BC_0063) %>%
  ggplot(aes(x=BC_0066, y=BC_0063)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"BC (ng/m3)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2) + 
  geom_smooth(method="lm") + 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
  # stat_poly_eq(formula = y~x, parse=T, coef.digits = 2,
  ...
  # colo.plot(x.variable = "BC_0066", y.variable = "BC_0063", 
  #           slope_digits = 2, alpha_value = 0.7)  


p[[2]] <- dt_stops3_colo_wide %>%
  drop_na(CO2_14, CO2_19) %>%
  ggplot(aes(x=CO2_14, y=CO2_19)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"CO2 (ppm)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2)+ 
  geom_smooth(method="lm") + 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
  ...
  # colo.plot(x.variable = "CO2_14", y.variable = "CO2_19", 
  #           alpha_value = 0.7)  
...
p[[3]] <- dt_stops3_colo_wide %>%
  ...
  # colo.plot(x.variable = "NO2_2", y.variable = "NO2_1",  
  #           slope_digits = 0, int_digits = 2, alpha_value = 0.7)  

p[[4]] <- dt_stops3_colo_wide %>%
  drop_na(PM25_176, PM25_205) %>%
  ggplot(aes(x=PM25_176, y=PM25_205)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"Neph (bscat/m)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2)+ 
  geom_smooth(method="lm") + 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
  ...
  # colo.plot(x.variable = "PM25_176", y.variable = "PM25_205", int_digits = 6, rmse.digits = 6, alpha_value = 0.7)  

p[[5]] <- dt_stops3_colo_wide %>%
  drop_na(PMDISC_3, PMDISC_8) %>%
  ggplot(aes(x=PMDISC_3, y=PMDISC_8)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"DiSCmini, UFP (pt/cm3)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2)+ 
  geom_smooth(method="lm") + 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
...
#colo.plot(x.variable = "PMDISC_3", y.variable = "PMDISC_8", alpha_value = 0.7)  

#that these are the only 2 non-screeend PTRAKS that were collocated 
p[[6]] <- dt_stops3_colo_wide %>%
  drop_na(PMPT_94, PMPT_93) %>%
  ggplot(aes(x=PMPT_94, y=PMPT_93)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"P-TRAK, UFP (pt/cm3)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2)+ 
  geom_smooth(method="lm") + 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
  ...
  #colo.plot(x.variable = "PMPT_94", y.variable = "PMPT_93",  alpha_value = 0.7)  

# that these are the only 2 screeend PTRAKS that were collocated 
p[[7]] <- dt_stops3_colo_wide %>%
  drop_na(PMPTSCREEN_94, PMPTSCREEN_3) %>%
  ggplot(aes(x=PMPTSCREEN_94, y=PMPTSCREEN_3)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"P-TRAK, screened, UFP (pt/cm3)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2)+ 
  geom_smooth(method="lm")+ 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
  ...
  #colo.plot(x.variable = "PMPTSCREEN_94", y.variable = "PMPTSCREEN_3", alpha_value = 0.7) 

p[[8]] <- dt_stops3_colo_wide %>%
  drop_na(PMSCAN_5, PMSCAN_3) %>%
  ggplot(aes(x=PMSCAN_5, y=PMSCAN_3)) + 
  geom_point(alpha=0.7) + 
  facet_wrap_equal(~"NanoScan, UFP (pt/cm3)") + 
  geom_abline(slope = 1, intercept = 0, linetype=2) + 
  geom_smooth(method="lm") + 
  coord_fixed() #+ 
  # #note labels are weird when use facet_wrap, but look correct when not using facetwrap
  ...
  #colo.plot(x.variable = "PMSCAN_5", y.variable = "PMSCAN_3", alpha_value = 0.7)  


print("Instrument Collocations")

ggarrange(plotlist = p)  

ggsave(file.path(image_path, "SI", "collocations.png"), height=9, width = 8) 
...
-same as above for ns bins during Feb-Mar 2019 since 

* showing data collected before `r drop_ns3` since this is when we know NS3 begins to fail   
  * things looked much worse when viewing all the data, for some bins more than others    

```{r}
ns_stops2_colo_wide %>%
  filter(date < drop_ns3
         ) %>%
  ...
  labs(title = "Comparison of collocated NanoScan bins",
       subtitle = "linear fit are for all the data on each panel"
  )
...
** collocation readings are similar.      
** Also, TRH_2 was never used on its own, so there is no need to calibrate readings. This approach is similar to our approach with other instruments. 

```{r}
 trh_stops_colo_wide %>%
  ggplot(aes(x=TRH_1, y=TRH_2)) + 
  facet_wrap_equal(~variable, scales="free") + 
  geom_abline(slope = 1, intercept = 0, linetype=2, alpha=0.4) +
  geom_point(alpha=0.5) +
  geom_smooth(se = F, method="lm",) +
  ...
  labs(title = "Comparison of collocated HOBO instruments")
...
- same as above for nanoscan bins, median, mode estimates 

```{r}

ns_bins_lm <- ns_stops2 %>%
  filter(#grepl("ns_total", variable),
         #use recent collocation data  
         date < drop_ns3_v2
         ) %>%
  select(time, instrument_id, variable, value) %>% 
  spread(instrument_id, value) %>% 
  ...
  nest() %>%     
  mutate(
    lm = map(data, ~lm(PMSCAN_3~PMSCAN_5, data = .x)),
    tidied = map(lm, tidy)
  ) %>%
  unnest(tidied) %>%
  select(term, estimate) %>%
...
# ns_bins_5_int <- ns_bins_lm$`(Intercept)`
# ns_bins_5_slope <- ns_bins_lm$PMSCAN_5
...
dt_stops4 <- dt_stops3 %>%
  ...
  mutate(
    ...
    mean_value = ifelse(grepl("neph", variable), mean_value*bscat_unit_conv*neph_slope + neph_int,
                          mean_value )
  )
...
ns_stops3 <- ns_stops2 %>%
  rename(value_c1 = value) %>%  
  # adjust earliest PMSCAN_3 readings based on PMSCAN_5 lm fit
  left_join(ns_bins_lm) %>%
  mutate(
    value = ifelse(grepl("PMSCAN_3", instrument_id) & date < drop_ns3_v2, 
                   (value_c1-int)/slope,
                   value_c1),
    # no calib curves for a few days ? use non-calibrated values in this case. the adjustment is small either way.
    value = ifelse(is.na(value), value_c1, value)
    ) %>%
  select(-c(int, slope))
...
t2 <- dt_stops4 %>%
  group_by(variable) %>%
  summarize(dt_stops4 = length(unique(stop_id)))

left_join(t1, t2) %>%
  mutate(stops_diff = dt_stops3 - dt_stops4) %>%
  kable(caption = "Stops before and after updating data. all backup CO2 readings are dropped b/c they appear to be unreliable") %>%
  kable_styling()
...
- same as above for NanoScan bins 
...
* backup instrument was never used on its own

```{r}
#the backup instrument was never used on it's own, so we can drop this
# trh_stops_colo %>%  
#   spread(instrument_id, value) %>%
#   filter(is.na(TRH_1) &  !is.na(TRH_2))

trh_stops2 <- trh_stops %>%
  filter(instrument_id == "TRH_1")
...
**Note that BH and 10W don’t have minute data. BH has rolling 1-hour averages updated every 6 minutes for PM2.5 (teoms), while 10W seems to report the same thing every minute for about an hour (~58 min).**
...
* There are various discrepancies between DOE and mobile monitoring observations    
  ...
  * instrument differences    
    ...
    * **Note that BH and 10W don’t have PM2.5 minute data. BH has rolling 1-hour averages updated every 6 minutes for PM2.5 (teoms), while 10W seems to report the same thing every minute for about an hour (~58 min).**          
...
aqs_colo_p <- dt_doe_l %>%
  ...
  geom_smooth(aes(x=DOE, y=MM), method = "lm", se = F, inherit.aes = F ) + 
   stat_poly_eq(aes(label =  paste(stat(eq.label),
                                  stat(rr.label), sep = "*\", \"*")),
...
same as above for manuscript (no CO or neph)

```{r}

aqs_colo_p %+% subset(dt_doe_l, grepl("BC|NO2|PM2.5", pollutant))

ggsave(file.path(image_path, "SI", "2min_aqs_colo.png"), width = 9, height = 5)
...
same as above but for manuscript

```{r}
annual_colo_p %+% subset(true_annual, grepl("BC|NO2|PM2.5", pollutant))

ggsave(file.path(image_path, "SI", "annual_aqs_colo.png"), width = 9, height = 5)
...
stop_tb <- dt_stops6 %>%
  #use original bscat units (not PM2.5 calibration) here
  mutate(value = ifelse(variable== "neph_bscat", value_raw, value)) %>%
  group_by(variable, #instrument_id
           ) %>%
  summarize(
    N = n(),
...
final_data <- dt_stops6 %>%
  select(-c(value_c1, value_raw)) %>%
  rename(median_value = value) %>%
  #relabel neph PM2.5 for clarification
  mutate(variable = ifelse(grepl("neph", variable), "pm2.5_ug_m3", variable))

all_stops <- length(unique(final_data$stop_id)) #value is same for original_data. #9047
...
print("mean vs median 2-min visit concentrations")

final_data %>%
  ggplot(aes(x=median_value, y=mean_value)) + 
  facet_wrap_equal(~variable, scales="free") + 
  geom_point(alpha=0.3) + 
  geom_abline(slope = 1, intercept = 0, linetype=2, alpha=0.5) 

final_data %>%
  mutate(diff = mean_value - median_value) %>%
  group_by(variable) %>%
  summarize(
    min = min(diff),
    mean = mean(diff),
...
```{r}
# second-level data
final_second_data <- final_data %>%
  select(stop_id, instrument_id) %>%
  left_join(dt2)
...
* some individual, high-resolution readings were dropped b/c of flags, but this did not impact the number of stop medians    
  * non-fatal flag: NO2: readings during automatic baseline checks        
  * fata flags: MA200 (BC): pump flow and optical saturation errors    
  * QC decision: screened P-TRAKS readings < 100 pt/cm3 (other instruments still had readings, so this did not impact the # of stops with readings)      

* these stop medians were dropped:     
  * readings outside the range of each instrument's reporting range     
  * NanoScans and non-screened P-TRAKS readings < 300 pt/cm3    
  * dropped all CO2_19 (backup instrument) readings b/c unreliable, it did not correlate well with primary instrument, and readings seemed generallyunstable. This reduced the number of CO2 readings when this instrument was used on its own  
  * dropped all CO data because it they did not correlate well across duplicate instruments or with DOE data.
...
s0 <- original_data %>%
  ...
  group_by(variable) %>%
  summarize(
    original_stops = length(unique(stop_id)),
    original_stops_pct = paste0(format(original_stops/all_stops*100, digits = 2, nsmall = 2), "%")
  )

s1 <- final_data %>%
  group_by(variable) %>%
  summarize(
    final_stops = length(unique(stop_id)),
    final_stops_pct = paste0(format(final_stops/all_stops*100, digits = 2, nsmall = 2), "%")
  )
...
full_join(s0, s1) %>%
  ...
  mutate(variable = ifelse(grepl("UFP", variable), paste0(variable, ", ", ufp_range_nm, "nm"), as.character(variable) )) %>%
  select(-ufp_range_nm) %>%
  #rbind(c("TOTAL", sum(.$original_stops), sum(.$final_stops))) %>%
  #mutate_at(vars(-variable), ~as.numeric(.)) %>%
  mutate(
    stops_dropped = original_stops - final_stops,
    percent_stops_dropped = paste0(round(stops_dropped/original_stops*100, 2), "%"),
     ) %>%
  relocate((contains("dropped")), .after = original_stops_pct ) %>%
  
  rbind(c("TOTAL", sum(.$original_stops), paste0(round(71131/(8*all_stops)*100, 2), "%"), 
          sum(.$stops_dropped), paste0(round(435/(71131)*100, 2), "%"),
          sum(.$final_stops), paste0(round(70696/(8*all_stops)*100, 2), "%")
          )) %>%
  mutate_at(vars(original_stops, stops_dropped, final_stops), ~as.numeric(.)) %>%
  
  kable(caption = paste0("Mobile monitoring stops. Percents for the original stop measurements and the final stops are based on the total number of stops in the campaign (", all_stops, "); percents for the stops dropped are based on the original stop measurements" ), 
        digits = 2, 
        col.names = c("", rep(c("N", "%"),3)),
        format.args = list(big.mark=', ')
        ) %>%
  kable_styling() %>%
  add_header_above( c("Pollutant", "Original Stop Measurements" =2, "Dropped Stop Measurements" =2, "Final Stop Measurements" = 2))
...
```{r}
#total row %s
paste0(round(71131/(8*all_stops)*100, 2), "%")
...
write.csv(final_data, file.path("Data", "Output", "stop_data.csv"), row.names = F)


# #second data
## no need to save this if nothing changes really
saveRDS(final_second_data, file.path("Data", "Output", "second_data.rda"))

# nanoscan bin stop data 
saveRDS(ns_stops3, file.path("Data", "Output", "ns_stop_data.rda"))

#temp data
saveRDS(trh_stops2, file.path("Data", "Output", "trh_stop_data.rda"))
...
### Future Ideas for QCing data

* Kerckhoffs 2021 (Nationwide UFP modeling from mobile monitoring) droppped CPC concentrations <= 500 ptc/m3; or if conc increased/decreased in one second by a factor of 10+ since these reflect malfunctions in the instruments (they cite Kerckhoffs 2016)
