...
* This script documents the quality control (QC) procedures taken to ensure data quality after the ACT TRAP mobile monitoring campaign. It is on [GitHub](github.com/magali17/OurCampaign.git).
...
room_calibrations_data <- readRDS(file.path("Data", "Original", "room_calibrations_data.rda")) %>%
  filter(
...
instrument_range <- dt0 %>%
  distinct(variable) %>%
  mutate(
    Min = c(0, 0, 0, 10^3, 0, 0, 0, 10^2, 0),
    #no max for neph
    Max = c(200, 5e3, 2e3, 10^6, 5e5, 10^6, NA, 10^6, 5e5),
    Units = c("ppm", "ppm", "ppb", "pt/cm3", "pt/cm3", "ng/m3", "bscat/m", "pt/cm3",  "pt/cm3")
...
```{r}
# time series 
calib_instrument_ids <- room_calibrations_data %>%
  filter(grepl("co_|co2|no2|neph_bscat", variable, ignore.case = T)) %>%
...
#join actual instrument readings
  left_join(room_calibrations_data) %>%
  drop_na(value)
...
```{r, test, eval=F}
test <- calibrations_table0 %>%
  
  filter(grepl("PM25_", instrument_id)) %>% 
...
test_room_calib_dt <- room_calibrations_data %>%
  filter(grepl("PM25_", instrument_id),
         #grepl("2019-11-13", runname)
         )

room_calibrations_data %>%
  group_by(variable, instrument_id) %>%
  summarize(
    min = min(time),
    max = max(time)
...
calibrations <- calibrations_table0 %>%
  #drop these data so not confused w/ no2 when merging
  filter(
    # Tim also collected these data from the CAPS NO2, but we don't need these
    !analyte %in% c("nox", "no"),
    ) %>%   
    #some instruments (esp NO2) have a lag response period. wait a few seconds before using the data
    mutate(timeint = timeint + start_lag,
...
```{r}
co2_neph_response <- 2.14e-5
```

* responses near `r co2_neph_response` were likely reading CO2 gas, which is associated with this response at T=293K/20C/68F and P=1013 mb for a CO2:Air ratio of 2.61 according to the [instrment manual](https://gml.noaa.gov/aero/instrumentation/pdfs/RR903_manual.pdf).    

* How the observed readings compare with 0 or with `r co2_neph_response` shows how far the neph is out of spec, so you could use those readings to adjust data collected prior to this calib check."

```{r}
# show all the data, even the times when only single concentrations (e.g., zero) exist
...
calibrations %>%
  ...
  group_by(variable, instrument_id, runname, date) %>%
  ungroup() %>%
  filter(grepl("neph", variable)) %>%
  #when nephs response near 2.14e-4, co2 gas was likely used (vs a filter or zero air)
  #mutate(conc = ifelse(value > 1e-5, co2_neph_response, conc)) %>% 
  group_by(instrument_id, Reference_gas = conc) %>%
  summarize(
    N = n(),
    Min = min(value),
    Median = median(value),
    IQR = IQR(value),
    Max = max(value),
    Mean_error = mean(value-conc)
  ) %>%
  kable(caption = "Neph responses to reference air. N = number of measured concentrations (1 per day)",
        #digits = 2
        ) %>%
  kable_styling()
...
```{r neph calibration plot, eval=F}
# plot all the data, even the times when only single concentrations (e.g., zero) exist
...
calibrations %>%
  ...
  filter(grepl("neph", variable)) %>%# View()
  #when nephs response was near 2.14e-4 on 2019-10-02 - I think CO2 gas must have been used (vs a filter or zero air)
 # mutate(conc = ifelse(value > 1e-5, co2_neph_response, conc)) %>% 
  
  ggplot(aes(x=conc, y=value, col=date, group=date)) + 
  facet_wrap(~instrument_id, scales="free") +
  theme(aspect.ratio = 1,
        legend.position = "none"
        ) +
  geom_hline(yintercept = c(0, co2_neph_response), linetype=2, alpha=0.5) +
  geom_vline(xintercept = c(0, co2_neph_response), linetype=2, alpha=0.5) +
  geom_line(stat="smooth", method = "lm", size = 0.5, alpha = 0.8) +
  geom_point(alpha=0.8) + 
  
   labs( 
     title = "Comparison of median neph responses to zero and 99.9% CO2 gas",
     subtitle= "Each colored line is a calibration date \nDashed lines are expected instrument responses based on reference air",
     x = "Reference Conc",
     y = "Instrument Response Conc (bscat/m)"
       )
...
# dt = dt0 %>% filter(instrument_id == "NO2_1") %>%
#   mutate(value2 = value)
# calibration_curves_df = calibration_curves %>% filter(instrument_id == "NO2_1")
...
### --> waiting for Amanda to update calibration table since it currently does not inclue zero's that occurred at the end of routes ?
...
### --> If readings are not near 0 or the noise is large, ask Edmund Elena what we should do about specific instrument. adjust backwards? 
...
calibrations %>%
  ...
  filter(
    str_detect(instrument_id, paste0("^", paste(pt_pollutants, collapse = "|"))),
    #conc==0,
    ) %>% 
  ggplot(aes(x=time, y=value, col=runname, shape=runname)) + 
  scale_shape_manual(values = c(1:length(unique(calibrations$runname)))) +
  facet_wrap(~runname+instrument_id, scales="free") + 
  geom_point(alpha=0.5) + 
  geom_hline(yintercept = 0, linetype="dashed")

#particle instruments
#p1 %+% subset(calibrations, str_detect(instrument_id, "PM") & !str_detect(instrument_id, "PM25"))


```

* same as above, but using 2-minute medians
...
low_value <- dt %>%
  group_by(variable, instrument_id) %>%
  summarize(low = quantile(value, low_q))
...
calibrations %>%
  ...
  geom_hline(aes(yintercept = low, col=paste0("Low Stop Conc\nQ", low_q))) +
  labs(title="instrument readings during 0 pt/cm3 concentration times (filter use)",
       col=""
       )
...
calibrations %>%
  ...
  ggplot(aes(x=instrument_id, fill=runname)) + 
  facet_wrap(~variable, scales="free") +
  geom_hline(yintercept = 0, linetype=2, alpha=0.5) +
  geom_boxplot(stat = "identity", aes(ymin=Qmin, lower=Q25, middle=Q50, upper=Q75, ymax=Qmax)) +
...
### --> are the nephs, BC and scan 3/5 probelematic?
...
### --> ? may decide to adjust particle instruments backwards since these are not adjusted like the gas instruments are otherwise? 
...
### --> ? Check other error codes (low flow etc). get the % (avg of binary) flag for any given stop. e.g., if 1% of the data has low flow error, may not need to worry  
...
* older sources that are slightly different 
  ...
  * Amanda's QAQC html only has status values (not always numerical) for some of the instruments (as of 2021-06-11)   
...
dt_stops <- dt %>%
  ...
  mutate(time = min(time)) %>%  
   group_by(runname, date, time, location, stop_id, site, instrument_id, variable, primary_instrument) %>%
  summarize(
    value = median(value),
...
```{r, eval=F}
# DON'T need to do this b/c most of this is in the plots already?

# * note, have to modify code if we ever have > 2 collocated duplicate instruments (e.g. if we keep all the PMPTSCREEN data above)


### --> recode. why does ma200 have NAs? pnc_screen has R2_reg of 1.00??
### --> edit code for CO 
...
colo3 <- dt_stops3_colo %>%
  group_by(time, site, variable) %>% #mutate(length=length(unique(instrument_id))) %>%View()
  mutate(colo_yes = length(unique(instrument_id)) >1) %>% #View()
  filter(colo_yes == TRUE ) %>%  #View()
  ungroup() %>%
...
for (i in seq_along(instruments)) {
  ...
  colo0 <- dt_stops3_colo_wide %>%
    ...
    select(runname, date, time, #location, 
...
```{r, fig.height=12, eval=F}
### --> update: 3 CO instruments weren't collocated all at once
### --> update colo data ??
...
#for(i in seq_along(ap)) {
  ...
  #p[[i]] <-  
    ...
    mutate(first_stop = min(time),
           ...
           first_stop = factor(first_stop)
           ) %>% 
    ungroup() %>% #View()
    
    ggplot(aes(x=time, y=value, 
               #col=factor(date),
               #linetype=instrument_id, 
               linetype=primary_instrument
               )) +
    facet_wrap(~variable, scales="free") +
    
      geom_smooth(aes(col= first_stop #factor(first_stop),
      #col = runname
                    ), se=F, alpha=0.8 ) +
...
* calibrate backup instruments when they deviate from the primary instrument using recent collocation data    
  - calibrate NanoScan (ns) 3 to ns 5 using data collected before `r drop_ns3_v2` since 3 tends to read low and ns 3 values after this will be dropped anyways
  - calibrate NO2_1 to NO2_2 using data collected before `r drop_no2_1`.     
    * Note that some NO2_1 values become negative and will be dropped later.         
...
### --> need to drop ns 3 values on/after 2020-02?
...
dt_stops4 <- dt_stops3 %>%
  ...
  filter(
    ...
    #drop 2020 PMSCAN_3 data after 2019 - data will be not used anyways
  ...
  mutate(
    ...
    value =
      # adjust PMSCAN_3 readings based on PMSCAN_5
      ifelse(grepl("PMSCAN_3", instrument_id), (value_c1-ns_5_int)/ns_5_slope,
             # adjust NO2_1 readings based on NO2_1
             ifelse(grepl("NO2_1", instrument_id), (value_c1-no2_2_int)/no2_2_slope,
...
### --> compare updated values to lab calibrations again. Are these better or at least not worse?
...
temp <- dt_stops4 %>%
  ...
  group_by(runname, date, time, location, stop_id, site, variable) %>%
  mutate(
    n = n(),
...
### --> do this for adjusted-specific instruments (e.g., NO2_1, ?NS3). compare valu_c1 to final value. are final values better or at least not worse? If adjustment is correct, it should be near 1-1 line. If this is true, we shoud keep negative values – this is our best estimate
