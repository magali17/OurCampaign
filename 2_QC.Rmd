...
dt <- dt0 %>%
  filter(
    #drop particle readings of 0 since these are often associated w/ alcohol wick issues
...
observations   
* many/most instruments have extreme values
...
### ---> calculate R2, RMSE between these; also mean or range of diff? 
...
* poor NO2 fit. NO2_1 had issues?    
* which instrument do we trust?   
...
* PMDISC_8 has some low readings. 
...
for (i in seq_along(instruments)) {
  ...
  colo0 <- dt_stops_wide %>%
    ...
    drop_na() %>%
    gather("instrument_id", "median_value", contains(instruments[i])) 
...
for(i in seq_along(ap)) {
  ...
  p[[i]] <- colo %>%
    ...
    ggplot(aes(x=time, y=median_value, col=instrument_id, linetype=primary_instrument)) +
    facet_wrap(~variable, scales="free") +
    geom_smooth() +
...
for(i in seq_along(ap)) {
  ...
  p[[i]] <- dt_stops %>%
    ...
    ggplot(aes(x=time, y=median_value, col=instrument_id,
               linetype=primary_instrument
               )) +
    geom_smooth() +
    facet_wrap(~variable, scales="free") +
...
#plot with all the data
p0 <- dt_stops %>%
    ggplot(aes(x=run_stop_no, y=median_value, col=instrument_id,
               linetype=primary_instrument
               )) +
   #geom_boxplot() + 
  geom_smooth() +
