...
calibrations %>%
  ...
  facet_wrap(~variable +ufp_instrument, scales="free") +
...
full_join(s0, s1) %>%
  variable_relabel() %>%
  mutate(variable = ifelse(grepl("PNC", variable), paste0(variable, ", ", ufp_instrument), as.character(variable) )) %>%
  select(-c(ufp_range_nm, ufp_instrument)) %>%
...
aqs_colo_p <- dt_doe_l %>%
  ...
  labs(x = "DOE Measurements",
       y = "Mobile Monitoring Measurements",
       col = "AQS Site "
       )
...
annual_colo_p <- true_annual %>%
 ...
 # geom_line(stat="smooth", method = "lm", size = 0.5, alpha = 0.5) +
    ...
    labs(
         ...
         y = "Estimated Annual Average\nFrom Repeated 2-Min Measures"
         )
