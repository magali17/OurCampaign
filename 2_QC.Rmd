...
### --> need to do?? *  dropped PMPT_94 when time >= "2019-04-18 11:48:00" & time < "2019-04-18 12:12:00" in LABC. Paper lab notebook states that this PTRAK had alcohol wick issues. Instrument 94 also reads higher than 93.
...
calibration_data <- readRDS(file.path("Data", "Original", 
                                            #"calibration_data.rda"
                                            "calibration_data.rda"
                                            )
                                  ) %>%
  rename(time = timeint)
...
instruments_calibrated <- unique(calibrations$instrument_id)
...
### --> see pt notes for: 2019-10-02 for PMPT_94, PMSCAN_5 
...
calibrations <- calibrations_table0 %>%
  ...
  filter(
    ...
    #some instruments (esp NO2) have a lag response period. wait a few seconds before using the data
    mutate(timeint =  if_else(instrument_id %in% calib_instrument_ids, 
                             timeint + gas_start_lag,
                             timeint + pt_start_lag),
           endtime = endtime-end_lag
           ) %>%  
    #one time gets dropped. there is an error/instrument restart note.
    filter(timeint < endtime) %>%
...
bc_errors %>%
  ...
  summarize(
    ...
    n_with_error = sum(value >0),
    prop_with_error = n_with_error/N
  ) %>%
  kable(caption="Distribution of the proportion of time during any given stop with MA200 error messages. N = number of stops.", digits = 2) %>%
  kable_styling()
...
* what is a low-level concentration? 
...
stop_tb %>%
  mutate_if(is.numeric, ~format(., scientific = T, digits = 2)) %>%
  kable(caption = "Distribution of stop medians. N = number of stops: 309 locations x visits/location.", 
        #digits = 2, 
        ) %>%
  kable_styling()
  
#use in zsero plots later
...
### --> use calibration_data instead of room_colo AND calcu 2-min medians: 
